...
public class EmailServiceImpl implements EmailService {
    ...
    public void confermaLezione(PrenotazioneLezione lezione) {
        ...
        try {
            ...
            helper.setFrom(from);
            helper.setTo(mailStudente);
            helper.setSubject("Conferma prenotazione lezione - " + lezione.getMateria());
    ...
    public void annullaLezione(PrenotazioneLezione lezione) {
        ...
        try {
            ...
            helper.setFrom(from);
            helper.setTo(mailStudente);
            helper.setSubject("Annullamento prenotazione lezione - " + lezione.getMateria());
            ...
            log.error("ERRORE INVIO EMAIL STUDENTE PER ANNULLAMENTO PRENOTAZIONE LEZIONE", e);
        }
    }
    ...
    private void notificaAdmin(LezioneResponseDto lezione, boolean isModifica) {
        ...
        try {
            ...
            MimeMessageHelper adminHelper = new MimeMessageHelper(
                    adminMessage,
                    MimeMessageHelper.MULTIPART_MODE_MIXED_RELATED,
                    StandardCharsets.UTF_8.name()
            );

            adminHelper.setFrom(from);
            adminHelper.setTo(notificationTo);
            adminHelper.setSubject("Nuova lezione prenotata - " + lezione.getMateria());
            ...
            // Logo inline
            ClassPathResource logo = new ClassPathResource("static/Logo.png");
            adminHelper.addInline("logoImage", logo);
    ...
    private void notificaStudente(LezioneResponseDto lezione, String mailStudente, boolean isModifica) {
        ...
        try {
            if (mailStudente != null && !mailStudente.isBlank()) {
                ...
                MimeMessageHelper studentHelper = new MimeMessageHelper(
                        studentMessage,
                        MimeMessageHelper.MULTIPART_MODE_MIXED_RELATED,
                        StandardCharsets.UTF_8.name()
                );

                studentHelper.setFrom(from);
                studentHelper.setTo(mailStudente);
                studentHelper.setSubject("Richiesta di prenotazione lezione ricevuta - " + lezione.getMateria());
                ...
                studentHelper.setText(studentHtml, true);

                // Logo inline (stesso file usato per l'email admin)
                ClassPathResource logo = new ClassPathResource("static/Logo.png");
                studentHelper.addInline("logoImage", logo);
